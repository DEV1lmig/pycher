...
from models import (
    User, UserCourseEnrollment, Course, Module, Lesson, Exercise,
    UserModuleProgress, UserLessonProgress, UserExerciseSubmission, UserExamAttempt, CourseExam,
    Progress as OldProgress # Import the old Progress model, aliasing if needed
)
from schemas import UserCreate, ProgressCreate, UserEnrollmentWithProgressResponse, LessonProgressDetailResponse, ExerciseProgressInfo # Add new schemas
    ...
    if all_exercises_correctly_submitted:
        ...
        if lesson_progress and not lesson_progress.is_completed:
            ...
            new_lesson_progress = UserLessonProgress(
                ...
                lesson_id=lesson_id,
                is_completed=True,
                started_at=datetime.utcnow(), # Or fetch from first exercise submission time
...
def _check_and_update_module_completion(db: Session, user_id: int, module_id: int):
    ...
    if all_lessons_completed:
        ...
        if not module_progress: # Create if doesn't exist (e.g. if module started implicitly)
            ...
            module_progress = UserModuleProgress(user_id=user_id, module_id=module_id, started_at=datetime.utcnow()) # Or derive started_at
            db.add(module_progress)
