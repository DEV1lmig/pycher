...
export function CourseCards({ courses }) {
  const { hasAccessToCourse, getCourseProgress, loading } = useCourseAccess();
              ...
              {isLocked && (
                <div className="absolute inset-0 bg-black/60 backdrop-blur-sm rounded-lg z-10 flex items-center justify-center">
                  <div className="text-center p-4">
                    ...
                    <p className="text-gray-400 text-xs mt-1">{accessInfo.reason}</p>
                  </div>
                </div>
              )}
                  ...
                  <div className="flex items-start justify-between mb-2">
                    ...
                    {isEnrolled && (
                      <Badge variant="secondary" className="ml-2">
                        {isCompleted ? "Completado" : `${Math.round(progress.progress_percentage)}%`}
                      </Badge>
                    )}
                  </div>
